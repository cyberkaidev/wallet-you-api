...
import { TransactionsService, TransactionsServiceProps } from "../interfaces/transactions-service.interface";

export async function transactionsService({ publicKey }: TransactionsServiceProps): Promise<TransactionsService> {
  const client = tatum();
  ...
  if (error?.code == "validation.failed") return {
    error: {
